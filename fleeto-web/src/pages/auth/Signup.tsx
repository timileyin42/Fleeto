...
import { registerOperator, googleSignIn } from '../../api/auth'
    ...
    <div className="min-h-screen bg-background flex items-center justify-center px-4">
      <div className="w-full max-w-[420px] bg-surface-container-lowest rounded-xl shadow-card p-xl border border-outline-variant/30">
        ...
        <div className="text-center mb-xl">
          ...
          <h2 className="text-h3 text-on-surface">Create your account</h2>
        </div>
        ...
        <div className="flex gap-2 mb-xl">
          {[1, 2, 3].map((s) => (
            <div
              key={s}
              className={`flex-1 h-1 rounded-full transition-colors ${s <= step ? 'bg-primary' : 'bg-surface-container-high'}`}
            />
          ))}
        </div>
        ...
        {step === 1 && (
          <div className="flex flex-col gap-lg">
            {/* Google — shown upfront */}
            <button
              type="button"
              onClick={handleGoogle}
              disabled={loading}
              className="w-full flex items-center justify-center gap-3 border border-outline-variant rounded-xl py-3 text-body-md font-medium text-on-surface hover:bg-surface-container transition-colors disabled:opacity-50"
            >
            ...
            <div className="flex items-center gap-md">
              ...
              <span className="text-body-sm text-on-surface-variant">or sign up with email</span>
              <div className="flex-1 h-px bg-outline-variant" />
            </div>

            <form onSubmit={handleNext} className="flex flex-col gap-lg">
              ...
              <button type="submit" className="w-full bg-primary text-on-primary text-label-caps font-semibold uppercase py-4 rounded-lg hover:opacity-90 transition-opacity">
                Continue
              </button>
            </form>
          </div>
        ...
        {step === 2 && (
          <form onSubmit={handleNext} className="flex flex-col gap-lg">
            ...
            <button type="button" onClick={() => setStep(1)} className="text-body-sm text-on-surface-variant hover:text-on-surface transition-colors text-center">
              ← Back
            </button>
          </form>
        )}
        ...
        {step === 3 && (
          <form onSubmit={handleSubmit} className="flex flex-col gap-lg">
            ...
            <button type="button" onClick={() => setStep(2)} className="text-body-sm text-on-surface-variant hover:text-on-surface transition-colors text-center">
              ← Back
            </button>
          </form>
        )}
        ...
        <p className="text-center text-body-sm text-on-surface-variant mt-lg">
          ...
          <Link to="/login" className="text-on-surface font-semibold hover:underline">Log in</Link>
        </p>
      </div>
    </div>
  )
